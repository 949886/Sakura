...
@property (nonatomic, strong) UIBannerScroller * scrollView;
...
-(void)initialize
{
    ...
    //Setup ScrollView
    _scrollView = [[UIBannerScroller alloc]initWithFrame:self.bounds];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    for (int i = page - 1; i <= page + 1; i++)
    {
        ...
        if (i >= 0 && i < count)
        {
            UIBannerCell *cell = [self cellAtIndex:i];
            if (!cell)
            {
                UIBannerCell *cell = [self dequeueReusableCell];
...
{
    UIBannerCell * cell = (UIBannerCell *)recognizer.view;
...
- (void)updateCellsForReuse
{
    ...
    for (UIBannerCell *cell in _cells)
    {
        if (cell.superview)
...
-(UIBannerCell *)dequeueReusableCell
{
    //若cells中存在未被加入superview的cell，则返回该cell复用
    UIBannerCell *cell = nil;
    for (cell in _cells)
        if (!cell.superview)
            return cell;

    //若找不到可以复用的cell，则创建新cell
    cell = [UIBannerCell new];
...
-(UIBannerCell *)cellAtIndex:(NSInteger)index
{
    for(UIBannerCell * cell in _cells)
        if(cell.index == index)
            return cell;
...
@implementation UIBannerScroller
